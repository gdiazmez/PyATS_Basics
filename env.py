...
class TriggerEnv(Trigger):
    ...
    def collect_environment_variables(self, uut, steps, message, testbed):
        ...
        output = uut.execute('admin show inventory all')
        ...
        with steps.start("Collect show environment power", continue_=True) as step:
            ...
            for k in output_parsed_inventory['module_name'].keys():
                m = re.search(r'power',k)
                if m:
                    loc = k[13:21]
                    #print ('location in inv for power-module is' , loc)
                    for line in output3:
                        line = line.strip()
                        m2 = re.search(loc,line)
                        if m2:
                            #print('there are power environment values for power-module at ', loc)
                            log.info("There are power environment values for power-module at {msg}".format(msg=loc))
                ...
                if m:
                    ...
                    for line in output4:
                        ...
                        if m2:
                            ...
                            log.info("There are fan environment values for fantray at {msg}".format(msg=loc))
